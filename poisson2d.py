...
X_left = torch.cat([x,y], dim=1) # (0, y)

x = torch.ones_like(y)
X_right = torch.cat([x,y], dim=1) # (1, y)

x = l * torch.rand(N_b, 1)
y = torch.zeros_like(x)
X_bottom = torch.cat([x,y], dim=1) # (x, 0)

y = torch.ones_like(x)
X_top = torch.cat([x,y], dim=1) # (x, 1)
...
# Error
cf3 = axes[2].contourf(Xn, Yn, error_n, levels=err_levels)
