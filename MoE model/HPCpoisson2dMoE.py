...
def run_seed(seed):
    ...
    X_left = torch.cat([x, y], dim=1)

    x = l * torch.ones_like(y)
    X_right = torch.cat([x, y], dim=1)

    x = l * torch.rand(N_b, 1)
    y = torch.zeros_like(x)
    X_bottom = torch.cat([x, y], dim=1)

    y = l * torch.ones_like(x)
    X_top = torch.cat([x, y], dim=1)
...
if __name__ == "__main__":
    ...
    if w_softa:
        np.savez(
            f"moe2dpos_expa_softa_{NUMBER_OF_SEEDS}seeds.npz",
            Xn=Xn,
            Yn=Yn,
            u_pred_n=u_pred_n,
            u_exact_n=u_exact_n,
        ...
        np.savez(
            f"moe2dpos_expanded_{NUMBER_OF_SEEDS}seeds.npz",
            Xn=Xn,
            Yn=Yn,
            u_pred_n=u_pred_n,
            u_exact_n=u_exact_n,
